...
- This checkout is Colter's private `coco` fork of Tailcall ForgeCode.
- Treat `https://github.com/tailcallhq/forgecode` as read-only upstream input.
- Never push branches or tags to Tailcall's upstream.
- Never create or edit upstream issues, comments, reviews, pull requests,
  discussions, releases, wiki pages, or other GitHub state.
- Only fetch from upstream and port upstream changes into this fork.
- Push coco changes only to the private `origin` fork after it is created.
