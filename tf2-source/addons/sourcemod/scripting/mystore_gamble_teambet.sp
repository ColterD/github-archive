...
public void OnPluginStart()
{
	...
	gc_iMinPlayer = AutoExecConfig_CreateConVar("mystore_teambet_player", "4", "min player for allow teambet", _, true, 0.0);
	gc_bAlive = AutoExecConfig_CreateConVar("mystore_teambet_alive", "1", "0 - Only dead player can start a game. 1 - Allow alive player to start a game.", _, true, 0.0);
	gc_iMin = AutoExecConfig_CreateConVar("mystore_teambet_min", "20", "Minium amount of credits to spend", _, true, 1.0);
	gc_iMax = AutoExecConfig_CreateConVar("mystore_teambet_max", "2000", "Maximum amount of credits to spend", _, true, 2.0);
...
public void OnClientDisconnect(int client)
{
	...
	MyStore_SetClientCredits(client, MyStore_GetClientCredits(client) + g_iBet[client], "Clean TeamBet pot");
	g_iBet[client] = 0;
	g_iTeam[client] = 0;
}
...
public Action Command_Bet(int client, int args)
{
	...
	else if (args == 2)
	{
		...
		Panel_TeamBet(client);
		MyStore_SetClientCredits(client, MyStore_GetClientCredits(client) - g_iBet[client], "Team Bet");
...
void Panel_TeamBet(int client)
{
	...
	Format(sBuffer, sizeof(sBuffer), "%t\n%t", "teambet", "Title Credits", g_sCreditsName, iCredits);
			...
			case 3, 4:
			{
				...
				else
				{
					...
					if (iCredits >= g_iBet[client])
					{
						...
						MyStore_SetClientCredits(client, iCredits - g_iBet[client], "TeamBet");
						CPrintToChat(client, "%s%t", g_sChatPrefix, "TeamBet Placed", iCredits, g_sCreditsName);
						FakeClientCommand(client, "play sound/%s", g_sMenuItem);
						Panel_TeamBet(client);
					}
...
public Action TeamBet_RoundStart(Event event, const char[] name, bool dontBroadcast)
{
	...
	for (int i = 1; i <= MaxClients; i++)
	{
		...
		MyStore_SetClientCredits(i, MyStore_GetClientCredits(i) + g_iBet[i], "Clean TeamBet pot");
...
public Action TeamBet_RoundEnd(Event event, const char[] name, bool dontBroadcast)
{
	...
	for (int i = 1; i <= MaxClients; i++)
	{
		...
		{
			MyStore_SetClientCredits(i, MyStore_GetClientCredits(i) + RoundFloat(g_iBet[i] * fMulti), "Win Bet");
