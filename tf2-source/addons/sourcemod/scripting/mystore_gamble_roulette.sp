...
public void OnPluginStart()
{
	...
	gc_fAutoStop = AutoExecConfig_CreateConVar("mystore_roulette_stop", "10.0", "Seconds a roll should auto stop", _, true, 0.0);
	gc_bAlive = AutoExecConfig_CreateConVar("mystore_roulette_alive", "1", "0 - Only dead player can start a game. 1 - Allow alive player to start a game.", _, true, 0.0);
	gc_iMin = AutoExecConfig_CreateConVar("mystore_roulette_min", "20", "Minium amount of credits to spend", _, true, 1.0);
	gc_iMax = AutoExecConfig_CreateConVar("mystore_roulette_max", "2000", "Maximum amount of credits to spend", _, true, 2.0);
...
public Action Command_Roulette(int client, int args)
{
	...
	else if (args == 2)
	{
		...
		MyStore_SetClientCredits(client, MyStore_GetClientCredits(client) - g_iBet[client], "Bet Roulette");
...
void Panel_Roulette(int client)
{
	...
	Format(sBuffer, sizeof(sBuffer), "%t\n%t", "roulette", "Title Credits", g_sCreditsName, iCredits);
...
void ProcessWin(int client, int bet, int multiply)
{
	...
	MyStore_SetClientCredits(client, MyStore_GetClientCredits(client) + iProfit, "Roulette Win");
