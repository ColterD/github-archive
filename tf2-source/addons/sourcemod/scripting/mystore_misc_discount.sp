...
public Action Timer_CheckDiscount(Handle timer)
{
	...
	for (int i = 0; i < g_iCount; i++)
	{
		...
		{
			g_bActive[i] = true;
			MyStore_LogMessage(0, LOG_EVENT, "Discount Started: %s", g_sName[i]);
		...
		{
			g_bActive[i] = false;
			MyStore_LogMessage(0, LOG_EVENT, "Discount Ended: %s", g_sName[i]);
...
public Action MyStore_OnGetEndPrice(int client, int itemid, int &price)
{
...
void LoadConfig()
{
	...
	BuildPath(Path_SM, sFile, sizeof(sFile), "configs/MyStore/discount.txt");
	KeyValues kv = new KeyValues("Discount");
	kv.ImportFromFile(sFile);
	if (!kv.GotoFirstSubKey())
	{
		SetFailState("Failed to read configs/MyStore/discount.txt");
	}
