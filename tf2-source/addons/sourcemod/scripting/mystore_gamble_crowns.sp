...
{
	name = "MyStore - Crowns gamble module",
...
public void OnPluginStart()
{
	...
	gc_fAutoStop = AutoExecConfig_CreateConVar("mystore_crowns_stop", "4.0", "Seconds a roll should auto stop", _, true, 0.0);
	gc_bAlive = AutoExecConfig_CreateConVar("mystore_crowns_alive", "1", "0 - Only dead player can start a game. 1 - Allow alive player to start a game.", _, true, 0.0);
	gc_iMin = AutoExecConfig_CreateConVar("mystore_crowns_min", "20", "Minium amount of credits to spend", _, true, 1.0);
	gc_iMax = AutoExecConfig_CreateConVar("mystore_crowns_max", "2000", "Maximum amount of credits to spend", _, true, 2.0);
	gc_iSmily = AutoExecConfig_CreateConVar("mystore_crowns_win_smily", "5", "Multiplier when win '㋛  ㋛  ㋛'", _, true, 1.0);
	gc_iBar = AutoExecConfig_CreateConVar("mystore_crowns_win_bar", "10", "Multiplier when win '㍴  ㍴  ㍴'", _, true, 1.0);
	gc_iCrown = AutoExecConfig_CreateConVar("mystore_crowns_win_crown", "25", "Multiplier when win '♛  ♛  ♛'", _, true, 1.0);
...
public Action Command_Crowns(int client, int args)
{
	...
	MyStore_SetClientCredits(client, MyStore_GetClientCredits(client) - g_iBet[client], "Bet Crowns");
	Start_Crowns(client);
...
void Panel_Crowns(int client)
{
	...
	Format(sBuffer, sizeof(sBuffer), "%t\n%t", "crowns", "Title Credits", g_sCreditsName, iCredits);
...
void ProcessWin(int client, int bet, int multiply)
{
	...
	MyStore_SetClientCredits(client, MyStore_GetClientCredits(client) + iProfit, "Crowns Win");
