...
public bool Config_Particle(KeyValues &kv, int itemid)
{
	...
	if (type == -1)
	{
		...
		MyStore_LogMessage(0, LOG_ERROR, "Particle '%s' - unknown type (must be aura,trail,spawn,kill or hit)", sBuffer);
		return false;
	}
	...
	{
		MyStore_LogMessage(0, LOG_ERROR, "Can't find particle file %s.", g_sFile[type][g_iCount]);
		return false;
	}
...
public int Equip_Particle(int client, int itemid)
{
	switch(g_iType[itemid])
	{
		case AURA, TRAIL:
		{
			...
			g_iEquipt[client][g_iType[itemid]] = MyStore_GetDataIndex(itemid);
